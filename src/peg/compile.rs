...
impl GrammarCompiler {
    ...
    fn node<'a>(
        ...
        node: &'a Tree,
        expected: &'static str,
    ) -> CompileResult<(&'a NodeMeta, &'a Tree)> {
    ...
    //     let (meta, m) = self.node(tree, "rule")?;
    //     if *meta.name != *"Rule" {
    //         return Err(CompileError::UnexpectedNodeName {
    //             expected: "Rule",
    //             found: meta.name.clone().into(),
    //         });
    //     }
    ...
    pub fn compile_grammar(&self, tree: &Tree) -> CompileResult<Grammar> {
        let (_meta, tree) = self.node(tree, "start")?;
        let (_meta, _tree) = self.node(tree, "grammar")?;
    ...
    //     let (name, m) = self.node(tree, "rule")?;
    //     if name != "Rule" {
    //         return Err(CompileError::UnexpectedNodeName {
    //             expected: "Rule",
    //             found: name.into(),
    ...
    //     let (name, m) = self.node(tree, "expression")?;
    //
    //     match name {
